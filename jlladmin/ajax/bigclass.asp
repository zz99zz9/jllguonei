...
Szb=request("Szb")
 select case SAction
  ...
  case "add"
  conn.execute"INSERT INTO [Table_ProSmallClass] (BigClassName,BigClassId,SmallClassName,OrderId,Szb) VALUES ('"&BCName&"','"&BCID&"','"&SCName&"','"&SOID&"','"&Szb&"')"
  case "mod"
  conn.execute"update [Table_ProSmallClass] set BigClassName='"&BCName&"',BigClassId='"&BCID&"',SmallClassName='"&SCName&"',OrderId='"&SOID&"',Szb='"&Szb&"' where SmallClassId="&SCID&""
  case "del"
  '可加判断是否删除
...
do while not srs.eof%><%if si>0 then %>,<%end if%>{"SCID":"<%=srs("SmallClassID")%>","SCName":"<%=srs("SmallClassName")%>","Szb":"<%=srs("Szb")%>","OId":"<%=srs("OrderId")%>"}<%srs.movenext
	si=si+1
    loop
	si=0
    srs.close
